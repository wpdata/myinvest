...
class MarketRotationStrategy(BaseStrategy):
    ...
    def generate_signal(
        ...
        """
        # DEBUG
        print(f"[DEBUG-STRATEGY] generate_signal called")
        print(f"[DEBUG-STRATEGY] current_position={current_position}")
        print(f"[DEBUG-STRATEGY] market_data len={len(market_data) if market_data is not None else 'None'}")
        print(f"[DEBUG-STRATEGY] index_data len={len(index_data) if index_data is not None else 'None'}")

        # 验证数据
        if index_data is None:
            logger.info("[EXIT] No index data")
            return {
                "action": "HOLD",
                "reason": "缺少大盘指数数据"
            }
        ...
        if not self.validate_data(index_data, required_rows=self.consecutive_days):
            logger.info(f"[EXIT] Data validation failed, len={len(index_data)}, required={self.consecutive_days}")
            return {
                "action": "HOLD",
                "reason": "指数数据不足"
            }
        ...
        logger.info(f"[DATA] idx_data length after sort: {len(idx_data)}")

        # 检查当前持仓状态
        current_holding = current_position or self.bond_symbol
        logger.info(f"[STATE] current_holding={current_holding}, etf_symbol={self.etf_symbol}")
        ...
        if current_holding == self.etf_symbol and position_entry_date:
            ...
            entry_idx = idx_data[idx_data['timestamp'] >= entry_date].index
            if len(entry_idx) > 0:
        ...
        logger.info(f"[DEBUG] idx_data长度: {len(idx_data)}, recent_days长度: {len(recent_days)}")
        logger.info(f"[DEBUG] last_n_changes: {last_n_changes.tolist()}")
        logger.info(f"[DEBUG] decline_threshold: {self.decline_threshold}")
        logger.info(f"[DEBUG] all_decline: {all(last_n_changes <= self.decline_threshold)}")
