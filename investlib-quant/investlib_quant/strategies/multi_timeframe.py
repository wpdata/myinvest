...
def register_strategy():
    ...
    strategy_info = StrategyInfo(
        ...
        logic="周线判断主趋势 + 日线寻找入场点 → 只在趋势方向一致时交易。周线上升+日线买入→强烈买入；周线下降+日线卖出→强烈卖出",

        parameters={
            "weekly_ma_short": {
                "default": 10,
                "description": "周线短期均线（周），用于判断中期趋势"
            },
            "weekly_ma_long": {
                "default": 20,
                "description": "周线长期均线（周），用于判断长期趋势"
            },
            "daily_ma_fast": {
                "default": 5,
                "description": "日线快速均线（天），用于捕捉短期信号"
            },
            "daily_ma_slow": {
                "default": 20,
                "description": "日线慢速均线（天），用于确认日线趋势"
            },
            "stop_loss_pct": {
                "default": 5.0,
                "description": "止损百分比"
...
# 准备数据：将周线数据对齐到日线
daily_data = fetch_daily_data('600519.SH', days=500)
weekly_data = resample_to_weekly(daily_data)
aligned_data = align_timeframes(daily_data, weekly_data)

# 生成信号
signal = strategy.generate_signal(aligned_data)

if signal and signal['action'] == 'BUY':
    print(f"买入信号: 入场价 ¥{signal['entry_price']:.2f}")
    print(f"止损: ¥{signal['stop_loss']:.2f}")
    print(f"止盈: ¥{signal['take_profit']:.2f}")
""".strip(),
